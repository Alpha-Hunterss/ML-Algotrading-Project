...
def cal_window_max(array, window_size, sampling_rate):
    ...
    for i in range(window_size, array.shape[0]):
        ...
        cA_positive_magnitude, _ = compute_fft(cA)
        cD_positive_magnitude, _ = compute_fft(cD)
        ...
        res[i, 0] = wavelet_dec_max
        res[i, 1] = wavelet_dec_range

        res[i, 2] = skew_wavelet_cA
        res[i, 3] = skew_wavelet_cD
...
def add_win_fe_base_func(
    ...
    for tf in timeframes:
        for w_size in window_sizes:
            ...
            new_columns.append(pd.DataFrame(res[:, 0].round(round_to), columns=col_WAVELET_dec_max, index=df.index))
            new_columns.append(pd.DataFrame(res[:, 1].round(round_to), columns=col_WAVELET_dec_range, index=df.index))
